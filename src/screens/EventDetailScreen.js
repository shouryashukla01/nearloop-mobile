...
export function EventDetailScreen({ navigation, route }) {
  ...
  return (
    <Screen>
      {event.imageUrl ? <Image source={{ uri: event.imageUrl }} style={{ width: "100%", height: 220, borderRadius: theme.radius }} /> : null}
      <Text style={{ color: theme.colors.coral, fontSize: type.eyebrow, fontWeight: "900", letterSpacing: 0 }}>{event.city}</Text>
      <Text style={{ fontSize: type.title, lineHeight: type.titleLine, fontWeight: "900" }}>{event.title}</Text>
      <Text style={{ fontSize: type.body, lineHeight: type.bodyLine, color: theme.colors.muted }}>{event.description}</Text>
      <Card style={{ gap: 10 }}>
