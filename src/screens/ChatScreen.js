...
export function ChatScreen({ route }) {
  ...
  const merge = useCallback((nextMessages) => {
    setMessages((current) => {
  ...
  }, [load, merge, otherUser.id, user.id]);
  ...
  return (
    <KeyboardAvoidingView
      ...
      style={{ flex: 1, backgroundColor: theme.colors.paper, paddingTop: insets.top }}
    >
      ...
      <FlatList
        ...
        contentContainerStyle={{ padding: 14, paddingBottom: 18, gap: 8 }}
        renderItem={({ item }) => {
      ...
      />
      <View style={{ flexDirection: "row", gap: 8, padding: 12, paddingBottom: Platform.OS === "android" ? 12 : Math.max(12, insets.bottom), backgroundColor: theme.colors.white, borderTopWidth: 1, borderTopColor: theme.colors.line }}>
